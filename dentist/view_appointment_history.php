...
if ($patient_id > 0) {
    ...
    $stmt = $pdo->prepare("
        ...
        WHERE p.id = ? AND a.dentist_id = ?
        GROUP BY p.id
    ");
    $stmt->execute([$patient_id, $dentist_id]);
    ...
    if ($patient) {
        ...
        $stmt = $pdo->prepare("
            ...
            FROM appointments a
            WHERE a.patient_id = ? AND a.dentist_id = ?
        ");
        $stmt->execute([$patient_id, $dentist_id]);
        ...
        $stmt = $pdo->prepare("
            SELECT a.*, 
                   u.first_name as frontdesk_first_name,
                   u.last_name as frontdesk_last_name
            FROM appointments a
            LEFT JOIN users u ON a.frontdesk_id = u.id
            WHERE a.patient_id = ? AND a.dentist_id = ?
...
renderDentistLayout('Appointment History', $pageContent, 'patients');
